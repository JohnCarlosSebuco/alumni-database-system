import { readFileSync } from "fs";
import { resolve, dirname } from "path";
import { fileURLToPath } from "url";
import { GoogleAuth } from "google-auth-library";

const __dirname = dirname(fileURLToPath(import.meta.url));

const serviceAccount = {
  type: "service_account",
  project_id: "alumni-database-system-8329f",
  private_key_id: "ee66cdb7235f4d4e70e640088c4c29db85624d7d",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@alumni-database-system-8329f.iam.gserviceaccount.com",
  client_id: "111418269735278415794",
};

const PROJECT_ID = "alumni-database-system-8329f";

async function getAccessToken() {
  const auth = new GoogleAuth({
    credentials: serviceAccount,
    scopes: ["https://www.googleapis.com/auth/firebase", "https://www.googleapis.com/auth/cloud-platform"],
  });
  const client = await auth.getClient();
  const token = await client.getAccessToken();
  return token.token;
}

async function deployFirestoreRules() {
  const rules = readFileSync(resolve(__dirname, "../firestore.rules"), "utf8");
  const token = await getAccessToken();

  const res = await fetch(
    `https://firebaserules.googleapis.com/v1/projects/${PROJECT_ID}/rulesets`,
    {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        source: {
          files: [{ name: "firestore.rules", content: rules }],
        },
      }),
    }
  );
  const ruleset = await res.json();
  if (!res.ok) throw new Error(`Ruleset create failed: ${JSON.stringify(ruleset)}`);

  const rulesetName = ruleset.name;

  // Release the ruleset to the default Firestore service
  const releaseRes = await fetch(
    `https://firebaserules.googleapis.com/v1/projects/${PROJECT_ID}/releases/cloud.firestore`,
    {
      method: "PATCH",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        release: { name: `projects/${PROJECT_ID}/releases/cloud.firestore`, rulesetName },
      }),
    }
  );
  const release = await releaseRes.json();
  if (!releaseRes.ok) throw new Error(`Release failed: ${JSON.stringify(release)}`);
  console.log("✓ Firestore rules deployed");
}

async function deployStorageRules() {
  const rules = readFileSync(resolve(__dirname, "../storage.rules"), "utf8");
  const token = await getAccessToken();

  const res = await fetch(
    `https://firebaserules.googleapis.com/v1/projects/${PROJECT_ID}/rulesets`,
    {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        source: {
          files: [{ name: "storage.rules", content: rules }],
        },
      }),
    }
  );
  const ruleset = await res.json();
  if (!res.ok) throw new Error(`Storage ruleset create failed: ${JSON.stringify(ruleset)}`);

  const rulesetName = ruleset.name;
  const bucket = `${PROJECT_ID}.firebasestorage.app`;

  const releaseRes = await fetch(
    `https://firebaserules.googleapis.com/v1/projects/${PROJECT_ID}/releases/firebase.storage%2F${encodeURIComponent(bucket)}`,
    {
      method: "PATCH",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        release: {
          name: `projects/${PROJECT_ID}/releases/firebase.storage/${bucket}`,
          rulesetName,
        },
      }),
    }
  );
  const release = await releaseRes.json();
  if (!releaseRes.ok) throw new Error(`Storage release failed: ${JSON.stringify(release)}`);
  console.log("✓ Storage rules deployed");
}

async function main() {
  console.log("Deploying security rules...");
  await deployFirestoreRules();
  await deployStorageRules();
  console.log("\nAll rules deployed successfully.");
  process.exit(0);
}

main().catch((err) => {
  console.error("Deploy failed:", err.message);
  process.exit(1);
});
