/**
 * One-time script to promote a Firebase user to admin.
 * Usage: node scripts/set-admin-role.mjs <user-email>
 *
 * Example: node scripts/set-admin-role.mjs admin@college.edu.ph
 */
...
if (!email) {
  console.error("Usage: node scripts/set-admin-role.mjs <user-email>");
  process.exit(1);
}
